...
def get_proxies(uri_IP):
	...
	# print(soup)
	#bs中的select方法通过类名查找，组合查找。   
	# (".odd > td:nth-of-type(n)")是指class为odd的标签里的第n个类型为td的子标签

	ip_list = soup.select("tr > td:nth-of-type(1)")               #select对象返回的是列表
...
def get_url_all(url, proxies):
	#注意复制粘贴的HEADERS里不能有奇异的符号
	headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.62 Safari/537.36'}
	r = requests.get(url=url, headers=headers, proxies=proxies)#requests库get方法获取HTML网页
...
if __name__=='__main__':
	uri_IP = "https://www.89ip.cn/index_1.html"
